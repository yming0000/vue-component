...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateRange(spinAim) {
        this.range.start = spinAim - this.range.space > 0 ? spinAim - this.range.space : 0;
        this.range.end = spinAim + this.range.space > this.values.length - 1 ? this.values.length - 1 : spinAim + this.range.space;
      },
      getRangeData(index) {
